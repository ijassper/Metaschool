...
[학생 호칭 및 높임 표현 최우선 규칙]
- 학생을 직접 부를 때는 제공된 전체 이름에 '학생'을 붙인 '{학생 이름} 학생' 또는 문맥에 맞는 학생 이름만 사용하세요. 같은 문단에서는 '학생'으로 바꾸거나 주어를 생략할 수 있습니다.
- 학생을 가리키는 2인칭 대명사 '당신'과 '귀하', 그리고 '학생님', '학습자님', '작성자님'은 어떤 경우에도 사용하지 마세요.
- 존댓말 설정은 피드백 문장의 종결을 정중하게 만드는 규칙이지 학생의 행동을 높이는 규칙이 아닙니다. 학생이 한 행동에는 주체 높임 선어말어미 '-시-'를 붙이지 마세요.
- '학생이 나눠주셨습니다/쓰셨습니다/보여주셨습니다/작성해 주셨습니다'라고 쓰지 말고 '학생이 나누었습니다/썼습니다/보여주었습니다/작성했습니다'처럼 표현하세요.
- 첫 인사는 이름이 있으면 '{학생 이름} 학생, 안녕하세요.' 또는 '{학생 이름} 학생이 작성한 글을 잘 읽었습니다.' 형식을 우선하세요.
- '당신의 주장'처럼 2인칭으로 지칭하지 말고 '{학생 이름} 학생이 제시한 주장', '이 주장', '글에서 제시한 주장'처럼 구체적으로 표현하세요.
- 결과를 출력하기 전에 금지 호칭과 학생 행동에 잘못 붙은 '주셨-, 하셨-, 쓰셨-, 보여주셨-' 표현이 없는지 점검하고 발견하면 반드시 고치세요.

[금지 사항]
'당신', '귀하', '학생님'처럼 실제 교사가 학생에게 잘 사용하지 않는 호칭을 쓰지 마세요.
...
def activity_analysis(request, activity_id):
    ...
    for student in target_students:
        ...
        if not answer:
            ...
            deadline_passed = bool(activity.deadline and timezone.now() > activity.deadline)
            status = answer.participation_status(deadline_passed=deadline_passed)
...
def api_process_db_row(request):
    if request.method == 'POST':
        try:
            ...
            requested_feedback_title = (str(body.get('feedback_title') or '').strip() or '피드백')[:150]
                    ...
                    if persist_feedback_session:
                        ...
                        with transaction.atomic():
                            ...
                            session_title = FeedbackSession.make_unique_title(
                                ...
                                title=requested_feedback_title,
                            )
